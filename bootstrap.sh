...
# On Windows (Git Bash), vagrant can fail with "The system cannot find the path specified"
# unless run from cmd.exe with a Windows working directory. run_vagrant runs any vagrant
# command via cmd when PLATFORM=windows.
run_vagrant() {
  ...
  cmd //c "cd /d \"$win_path\" && vagrant $*"
}

# Same for vagrant ssh (needs different quoting for the remote command).
run_vagrant_ssh() {
  if [[ "$PLATFORM" != "windows" ]]; then
    vagrant ssh "$@"
    return $?
...
if [[ "${EVENT}" == "up" ]]; then
  ...
  VAGRANT_UP_OUTPUT=$(run_vagrant ${EVENT} --provider=virtualbox 2>&1 | tee /dev/tty)
