...
- **Kubernetes 내부 네트워크**: `192.168.86.0/24` (VM 간 통신)
...
#### my-ubuntu
- `kube-master`: 192.168.86.100 (eth1), 192.168.0.100 (eth2)
- `kube-node-1`: 192.168.86.101 (eth1), 192.168.0.101 (eth2)
- `kube-node-2`: 192.168.86.102 (eth1), 192.168.0.102 (eth2)

#### my-mac
- `kube-master2`: 192.168.86.103 (eth1), 192.168.0.103 (eth2)
- `kube-node2-1`: 192.168.86.104 (eth1), 192.168.0.104 (eth2)
- `kube-node2-2`: 192.168.86.105 (eth1), 192.168.0.105 (eth2)

#### my-mac2
- `kube-master3`: 192.168.86.106 (eth1), 192.168.0.106 (eth2)
- `kube-node3-1`: 192.168.86.107 (eth1), 192.168.0.107 (eth2)
...
config.vm.define "kube-master2" do |master|
  ...
  master.vm.network "public_network", bridge: "en0: Wi-Fi (AirPort)", ip: "192.168.86.103"
  # 호스트 접근용 네트워크 추가
...
config.vm.define "kube-master3" do |master|
  ...
  # Kubernetes 네트워크 (my-ubuntu와 동일한 물리 네트워크)
  master.vm.network "public_network", bridge: "en0: Wi-Fi (AirPort)", ip: "192.168.86.106"
  # 호스트 접근용 네트워크 추가
...
config.vm.define "kube-node3-1" do |node|
  ...
  # Kubernetes 네트워크
  node.vm.network "public_network", bridge: "en0: Wi-Fi (AirPort)", ip: "192.168.86.107"
  # 호스트 접근용 네트워크 추가
...
- 192.168.86.x 네트워크가 Wi-Fi 네트워크에 존재
...
sudo bash /vagrant/scripts/local/configure-static-ip.sh eth1 <k8s-ip> 255.255.255.0 <k8s-gateway> "8.8.8.8 8.8.4.4"

# 호스트 접근용 인터페이스에 static IP 설정
sudo bash /vagrant/scripts/local/configure-static-ip.sh eth2 <host-ip> 255.255.255.0 <host-gateway> "8.8.8.8 8.8.4.4"
```
...
# Kubernetes 네트워크 인터페이스에 static IP 설정
sudo bash /vagrant/scripts/local/configure-static-ip.sh eth1 192.168.86.106 255.255.255.0 192.168.86.1 "8.8.8.8 8.8.4.4"

# 호스트 접근용 인터페이스에 static IP 설정
sudo bash /vagrant/scripts/local/configure-static-ip.sh eth2 192.168.0.106 255.255.255.0 192.168.0.1 "8.8.8.8 8.8.4.4"
```
...
|--------|---------|------------|-------------|
| my-ubuntu | kube-master | 192.168.86.100 | 192.168.0.100 |
| my-ubuntu | kube-node-1 | 192.168.86.101 | 192.168.0.101 |
| my-ubuntu | kube-node-2 | 192.168.86.102 | 192.168.0.102 |
| my-mac | kube-master2 | 192.168.86.103 | 192.168.0.103 |
| my-mac | kube-node2-1 | 192.168.86.104 | 192.168.0.104 |
| my-mac | kube-node2-2 | 192.168.86.105 | 192.168.0.105 |
| my-mac2 | kube-master3 | 192.168.86.106 | 192.168.0.106 |
| my-mac2 | kube-node3-1 | 192.168.86.107 | 192.168.0.107 |

### Static IP 설정 원리

`configure-static-ip.sh` 스크립트는 VM 내부에서 netplan을 사용하여 Static IP를 설정합니다:
...
192.168.86.0/24 dev eth1 scope link
```
...
**자동 업데이트:**
```bash
sudo sed -i.bak 's/192.168.86.200/192.168.0.210/g' /etc/hosts
```
...
**원인:**
- MacBook (192.168.0.133)과 Ingress IP (192.168.86.200)가 다른 서브넷
- 네트워크 라우팅 불가
...
- my-ubuntu의 kube-master(192.168.86.100)와 통신 불가
- containerd 설치 실패
...
# my-mac2의 VM에서 실행
vagrant ssh kube-master3
ping -c 2 192.168.86.100  # my-ubuntu의 kube-master
...
- [ ] eth1 (192.168.86.x)와 eth2 (192.168.0.x) 인터페이스 존재 확인
...
### 스크립트 위치
- Static IP 설정: `scripts/local/configure-static-ip.sh`
- 통합 적용 스크립트: `scripts/local/apply-static-ip.sh`
...
- **2026-01-23**: my-mac2의 VM들 네트워크 구성 문제 해결 (private_network → public_network)
- **2026-01-23**: Calico 설정 불일치 문제 해결
- **2026-01-22**: MetalLB IP 풀을 192.168.0.x로 변경하여 MacBook에서 직접 접근 가능하도록 수정
- **2026-01-22**: 모든 VM에 Static IP 설정 적용 완료
- **2026-01-22**: Jenkins Ingress 포트를 8080으로 수정
- **2026-01-22**: 문서 통합 및 통합 스크립트 생성

---

## 요약

이 문서는 분산된 Kubernetes 클러스터의 네트워크 구성을 설명합니다:

1. **Vagrantfile 구성**: `public_network` 사용으로 물리 네트워크 브리지
2. **Static IP 설정**: VM 재시작 후에도 IP 유지
3. **MetalLB 구성**: LoadBalancer 서비스에 192.168.0.x IP 할당
4. **Ingress 접근**: MacBook에서 직접 접근 가능
5. **네트워크 분리**: Kubernetes 내부(192.168.86.x)와 호스트 접근(192.168.0.x) 분리
6. **Calico 설정**: 기존 클러스터와 일치하는 설정 필요

이 구성을 통해 안정적이고 접근 가능한 Kubernetes 클러스터를 운영할 수 있습니다.
