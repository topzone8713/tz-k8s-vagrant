...
# Remove existing containerd if present to avoid conflicts
sudo apt-get remove -y containerd 2>/dev/null || true
sudo apt-get install docker-ce docker-ce-cli containerd.io -y

cp -Rf scripts/local/config.cfg /root/.ssh/config

# Copy resources to /root/.k8s (required for prop function)
if [ -d /vagrant/resources ]; then
  echo "Copying /vagrant/resources to /root/.k8s..."
  sudo rm -Rf /root/.k8s
  sudo cp -Rf /vagrant/resources /root/.k8s
  sudo chown -R root:root /root/.k8s
  echo "✓ /root/.k8s directory created successfully"
  if [ -f /root/.k8s/project ]; then
    echo "✓ /root/.k8s/project file exists"
  else
...
sudo sed -i "s/\$KUBELET_EXTRA_ARGS/\$KUBELET_EXTRA_ARGS --node-ip=192.168.0.100/g" /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
...
#sudo mkdir /data
#mount -t nfs -vvvv 192.168.0.100:/homedata /data
#echo '192.168.0.100:/homedata /data  nfs      defaults    0       0' >> /etc/fstab
